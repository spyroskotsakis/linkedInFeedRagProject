# QianfanLLMEndpoint — 🦜🔗 LangChain  documentation

**URL:** https://python.langchain.com/api_reference/community/llms/langchain_community.llms.baidu_qianfan_endpoint.QianfanLLMEndpoint.html
**Word Count:** 2290
**Links Count:** 487
...
_class _langchain\_community.llms.baidu\_qianfan\_endpoint.QianfanLLMEndpoint[\[source\]](https://python.langchain.com/api_reference/_modules/langchain_community/llms/baidu_qianfan_endpoint.html#QianfanLLMEndpoint)\#     
...
Instantiate:                    from langchain_community.llms import QianfanLLMEndpoint          llm = QianfanLLMEndpoint(         model="ERNIE-3.5-8K",         # api_key="...",         # secret_key="...",         # other params...     )     

Invoke:                    input_text = "用50个字左右阐述，生命的意义在于"     llm.invoke(input_text)                    '生命的意义在于体验、成长、爱与被爱、贡献与传承，以及对未知的勇敢探索与自我超越。'     

Stream:                    for chunk in llm.stream(input_text):         print(chunk)                    生命的意义 | 在于不断探索 | 与成长 | ，实现 | 自我价值，| 给予爱 | 并接受 | 爱， | 在经历 | 中感悟 | ，让 | 短暂的存在 | 绽放出无限 | 的光彩 | 与温暖 | 。                    stream = llm.stream(input_text)     full = next(stream)     for chunk in stream:         full += chunk     full                    '生命的意义在于探索、成长、爱与被爱、贡献价值、体验世界之美，以及在有限的时间里追求内心的平和与幸福。'     

Async:                    await llm.ainvoke(input_text)          # stream:     # async for chunk in llm.astream(input_text):     #    print(chunk)          # batch:     # await llm.abatch([input_text])                    '生命的意义在于探索、成长、爱与被爱、贡献社会，在有限的时间里追寻无限的可能，实现自我价值，让生活充满色彩与意义。'     

Note

QianfanLLMEndpoint implements the standard [`Runnable Interface`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.base.Runnable.html#langchain_core.runnables.base.Runnable "langchain_core.runnables.base.Runnable"). 🏃
...
_classmethod _validate\_environment\(
    ...
    _values : Dict_, \) → Dict[\[source\]](https://python.langchain.com/api_reference/_modules/langchain_community/llms/baidu_qianfan_endpoint.html#QianfanLLMEndpoint.validate_environment)\#     
...
Examples using QianfanLLMEndpoint
