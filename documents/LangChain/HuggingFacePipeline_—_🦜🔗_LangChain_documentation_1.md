# HuggingFacePipeline — 🦜🔗 LangChain  documentation

**URL:** https://python.langchain.com/api_reference/huggingface/llms/langchain_huggingface.llms.huggingface_pipeline.HuggingFacePipeline.html
**Word Count:** 2182
**Links Count:** 261
...
_class _langchain\_huggingface.llms.huggingface\_pipeline.HuggingFacePipeline[\[source\]](https://python.langchain.com/api_reference/_modules/langchain_huggingface/llms/huggingface_pipeline.html#HuggingFacePipeline)\#     
...
Only supports text-generation, text2text-generation, image-text-to-text, summarization and translation for now.

Example using from\_model\_id:                    from langchain_huggingface import HuggingFacePipeline     hf = HuggingFacePipeline.from_model_id(         model_id="gpt2",         task="text-generation",         pipeline_kwargs={"max_new_tokens": 10},     )     

Example passing pipeline in directly:                    from langchain_huggingface import HuggingFacePipeline     from transformers import AutoModelForCausalLM, AutoTokenizer, pipeline          model_id = "gpt2"     tokenizer = AutoTokenizer.from_pretrained(model_id)     model = AutoModelForCausalLM.from_pretrained(model_id)     pipe = pipeline(         "text-generation", model=model, tokenizer=tokenizer, max_new_tokens=10     )     hf = HuggingFacePipeline(pipeline=pipe)     

Note

HuggingFacePipeline implements the standard [`Runnable Interface`](https://python.langchain.com/api_reference/core/runnables/langchain_core.runnables.base.Runnable.html#langchain_core.runnables.base.Runnable "langchain_core.runnables.base.Runnable"). 🏃
...
_param _batch\_size _: int_ _ = 4_\#     
...
The model name. If not set explicitly by the user, it will be inferred from the provided pipeline \(if available\). If neither is provided, the DEFAULT\_MODEL\_ID will be used.

_param _model\_kwargs _: dict | None_ _ = None_\#     

Keyword arguments passed to the model.

_param _pipeline\_kwargs _: dict | None_ _ = None_\#     
...
_classmethod _from\_model\_id\(
    ...
    _model\_id : str_,     _task : str_,     _backend : str = 'default'_,     _device : int | None = None_,     _device\_map : str | None = None_,     _model\_kwargs : dict | None = None_,     _pipeline\_kwargs : dict | None = None_,     _batch\_size : int = 4_,     _\*\* kwargs: Any_, \) → HuggingFacePipeline[\[source\]](https://python.langchain.com/api_reference/_modules/langchain_huggingface/llms/huggingface_pipeline.html#HuggingFacePipeline.from_model_id)\#     
...
Parameters:     
  ...
  * **device** \(_int_ _|__None_\)

  * **device\_map** \(_str_ _|__None_\)

  * **model\_kwargs** \(_dict_ _|__None_\)

  * **pipeline\_kwargs** \(_dict_ _|__None_\)
