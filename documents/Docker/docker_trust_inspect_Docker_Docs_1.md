# docker trust inspect | Docker Docs
...
If signers are set up for the repository via other `docker trust` commands, `docker trust inspect` includes a `Signers` key:               $ docker trust inspect my-image:purple     
