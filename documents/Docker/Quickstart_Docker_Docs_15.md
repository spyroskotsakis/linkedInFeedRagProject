...
With the containerd image store enabled, rebuild the image with a new `v3` tag. This time, add the `--provenance=true` and `--sbom=true` flags.               $ docker build --provenance=true --sbom=true --push -t <ORG_NAME>/scout-demo:v3 .     
