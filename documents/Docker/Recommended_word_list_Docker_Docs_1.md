# Recommended word list | Docker Docs

**URL:** https://docs.docker.com/contribute/style/recommended-words/
**Word Count:** 539
**Links Count:** 82
...
Try to avoidÂ `below` when referring to an example or table on a documentation page. If required, useÂ `following` instead.
...
Use one word forÂ `checkbox`. Don't useÂ `check box`.

YouÂ selectÂ \(notÂ checkÂ orÂ enable\) andÂ clearÂ \(notÂ deselectÂ orÂ disable\) checkboxes.

#### click

Don't useÂ `click`. Instead, useÂ `select` with buttons, links, menu items, and lists.

SelectÂ applies to more devices, whileÂ click is more specific to a mouse.
...
_In Docker Desktop 4.1 and higherâ¦_ or _In Docker Desktop 4.1 and aboveâ¦_
